            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                <button onClick={() => update({ granularity: 'week' })}
                  ...
                  ${granularity === 'week' ? 'bg-indigo-600 text-white' : dark ? 'bg-[#20232E] text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                  주간
                </button>
                <button onClick={() => update({ granularity: 'month' })}
                  className={`text-xs px-2.5 py-1 rounded-lg transition-colors
                  ${granularity === 'month' ? 'bg-indigo-600 text-white' : dark ? 'bg-[#20232E] text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                  월간
                </button>
              </div>
            </div>
                ...
                <thead>
                  <tr className={dark ? 'bg-[#20232E]' : 'bg-slate-50'}>
                    <th className={`text-left px-3 py-2 font-semibold sticky left-0 z-10
                      ${dark ? 'bg-[#20232E] text-slate-400' : 'bg-slate-50 text-slate-500'}`}>
                      코호트
                    </th>
                    <th className={`text-right px-3 py-2 font-semibold ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                      규모
                    </th>
            ...
            <div className={`rounded-xl border p-4
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={averages}>
                  ...
                  <XAxis dataKey="period" tick={{ fontSize: 10, fill: dark ? '#64748B' : '#94A3B8' }} />
                  <YAxis domain={[0, 100]} tick={{ fontSize: 10, fill: dark ? '#64748B' : '#94A3B8' }}
                    tickFormatter={v => v + '%'} />
