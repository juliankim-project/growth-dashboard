...
export default function Sidebar({ nav, setNav, dark, toggleDark }) {
  ...
  const [open, setOpen] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(STORAGE_OPEN) || '{}')
    } catch { return {} }
  })
  ...
  /* 섹션 클릭: 토글 + 자동으로 첫 서브 선택 */
  const toggleSection = (section) => {
    const isOpen = !!open[section.id]
    setOpen(prev => ({ ...prev, [section.id]: !isOpen }))
    if (!isOpen) {
      // 펼칠 때 첫 서브메뉴로 이동
      setNav({ section: section.id, sub: section.subs[0].id })
    }
  }
  ...
  return (
    <aside className={`
      flex flex-col w-[220px] min-h-screen shrink-0 border-r transition-colors duration-200
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className={`
                  w-full flex items-center gap-2 px-2 py-2.5 rounded-lg text-xs
              ...
              >
                <GripVertical
                  ...
                  className={`shrink-0 cursor-grab active:cursor-grabbing
                    ${dark ? 'text-slate-700 group-hover:text-slate-500' : 'text-slate-300 group-hover:text-slate-400'}`}
                />
                <Icon name={sec.icon} size={14}
                  className={isActive ? 'text-indigo-500' : 'opacity-80'} />
                <span className="font-semibold flex-1">{sec.label}</span>
                <ChevronDown
                  size={12}
                    ...
                    return (
                      ...
                      >
                        <Icon name={sub.icon} size={12}
                          className={subActive ? 'text-white' : 'opacity-60'} />
                        <span className={subActive ? 'font-semibold' : 'font-medium'}>
                          {sub.label}
                        </span>
                      </button>
                    )
