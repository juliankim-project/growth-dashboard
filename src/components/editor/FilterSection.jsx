    ...
    <div className={`rounded-xl border ${dark ? 'border-[#A1BDD914]' : 'border-slate-200'}`}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {activeCount > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#579DFF] text-white font-bold">{activeCount}</span>}
      ...
      {open && (
        <div className={`border-t overflow-x-auto ${dark ? 'border-[#A1BDD914]' : 'border-slate-100'}`}>
          <div className="flex min-w-max">
            ...
            {COLS.map((col, ci) => {
              ...
              const bdrR = ci < COLS.length - 1 ? (dark ? 'border-r border-[#A1BDD914]' : 'border-r border-slate-200') : ''
              return (
                <div key={col.key} className={`w-36 flex flex-col shrink-0 ${bdrR}`}>
                  <div className={`flex items-center justify-between px-2.5 py-1.5 border-b shrink-0
                    ...
                    {col.sel.length > 0 && !isGroupByDim && (
                      <button onClick={() => clearDim(col.key)} className="text-[9px] text-[#579DFF] hover:text-[#579DFF] ml-1 shrink-0">해제</button>
                    )}
                  </div>
                  {onGroupByChange && (
                    <button onClick={() => onGroupByChange(isGroupByDim ? null : col.key)}
                      className={`text-[10px] px-2.5 py-1 text-center border-b w-full transition-colors
                        ...
                        ${isGroupByDim ? 'bg-violet-500/15 text-violet-400 font-semibold'
                          : dark ? 'text-slate-600 hover:text-slate-400 hover:bg-[#22272B]' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}>
                          ...
                          <button key={v} onClick={() => !dimmed && toggle(col.key, v)} title={v}
                            className={`text-[11px] px-2.5 py-[5px] text-left w-full transition-colors leading-snug break-words
                              ${dimmed ? (dark ? 'text-slate-700 cursor-default' : 'text-slate-300 cursor-default')
                                : isOn ? (dark ? 'bg-[#579DFF]/15 text-[#579DFF] font-semibold' : 'bg-[#E9F2FF] text-[#0C66E4] font-semibold')
                                  : dark ? 'text-slate-400 hover:bg-[#22272B] hover:text-slate-200' : 'text-slate-600 hover:bg-slate-50'}`}>
                            {!dimmed && isOn ? '✓ ' : ''}{v}
                          </button>
                        )
                      })
