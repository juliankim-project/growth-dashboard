    ...
    <div className={`min-h-screen ${t.bg}`}>
      ...
      <div className={`sticky top-0 z-20 ${dark ? 'bg-[#1D2125]/95' : 'bg-slate-50/95'} backdrop-blur-sm border-b ${t.border}`}>
        <div className="px-6 pt-4 pb-2">
          <h1 className={`text-lg font-bold ${t.text}`}>🏠 지점별 분석</h1>
        </div>
          ...
          <div className="flex items-center gap-1 ml-auto">
            {['branch', 'area'].map(mode => (
              <button key={mode} onClick={() => setViewMode(mode)}
                className={`text-sm px-2.5 py-0.5 rounded font-medium transition-all
                  ${viewMode === mode ? 'bg-[#0C66E4] text-white' : dark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-800'}`}>
                {mode === 'branch' ? '지점별' : '권역별'}
              </button>
            ))}
          </div>
          <span className={`text-sm ${t.muted}`}>{fmtNum(totalReservations)}건</span>
        </div>
      </div>
      ...
      <div className="px-6 pt-5 pb-8 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            { label: '게스트', value: fmtNum(totalGuests) + '명', icon: <Users size={16}/>, color: 'text-blue-500 bg-blue-500/10' },
            { label: '예약건', value: fmtNum(totalReservations) + '건', icon: <Building2 size={16}/>, color: 'text-violet-500 bg-violet-500/10' },
            { label: '크로스구매', value: fmtNum(crossPurchase.multiAreaGuests) + '명', icon: <ArrowRightLeft size={16}/>, color: 'text-orange-500 bg-orange-500/10' },
          ].map((kpi, i) => (
                ...
                {(viewMode === 'branch' ? branchStats : areaStats).map((row, i) => {
                  ...
                  return (
                    <tr key={i} className={`border-t ${t.border} ${dark ? 'hover:bg-[#2C333A]' : 'hover:bg-slate-50'}`}>
                      {viewMode === 'branch' ? (
                        <>
                          <td className={`px-4 py-2.5 font-medium truncate max-w-[140px] ${t.text}`}>{row.branch}</td>
                          <td className={`px-4 py-2.5 ${t.muted}`}>{row.area}</td>
                          ...
                          <td className={`px-4 py-2.5 ${t.muted}`}>{fmtKRW(row.avgRevenue)}</td>
        ...
        <div className={`rounded-xl border overflow-hidden ${t.card} ${t.border}`}>
          <div className={`px-5 py-3 border-b ${t.border} flex items-center justify-between`}>
            <h2 className={`text-sm font-semibold ${t.text}`}>📈 예약 페이스</h2>
            <div className="flex items-center gap-1.5">
              <CalendarDays size={14} className={t.muted} />
              <select value={paceMonth} onChange={e => setPaceMonth(e.target.value)}
                className={`text-sm rounded px-2 py-0.5 border outline-none ${t.input} ${t.inputFocus}`}>
          ...
          <div className="p-4">
            {bookingPace && bookingPace.totalCount > 0 ? (
              <>
                <div className={`text-xs mb-2 ${t.muted}`}>
                  ...
                  {bookingPace.buckets.map((bucket, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2">
                        <span className={`text-sm font-medium w-12 ${t.text}`}>{bucket.label}</span>
                        ...
                        <span className={`text-sm font-bold w-10 text-right ${sharePct > 25 ? 'text-blue-400' : t.text}`}>{fmtPct(sharePct)}</span>
                        <span className={`text-xs w-20 text-right ${t.muted}`}>{fmtKRW(bucket.revenue)}</span>
                      </div>
                    )
                  })}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className={`rounded-xl border overflow-hidden ${t.card} ${t.border}`}>
            <div className={`px-5 py-3 border-b ${t.border}`}>
              <h2 className={`text-sm font-semibold ${t.text}`}>🔀 크로스구매</h2>
              <p className={`text-xs ${t.muted}`}>다른 권역도 이용한 게스트 수</p>
            </div>
            <div className="overflow-x-auto p-3">
              {crossPurchase.areas.length > 0 ? (
                <table className="text-xs">
                  <thead>
                    <tr>
                      <th className={`px-2 py-1.5 text-left ${t.sub}`}>↓/→</th>
                      {crossPurchase.areas.map(a => (
                        <th key={a} className={`px-2 py-1.5 text-center ${t.sub}`}>{a}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {crossPurchase.areas.map(from => {
                      ...
                      return (
                        <tr key={from}>
                          <td className={`px-2 py-1.5 font-medium whitespace-nowrap ${t.text}`}>{from}</td>
                          {crossPurchase.areas.map(to => {
                            ...
                            const bg = from === to
                              ...
                              : ''
                            return (
                              <td key={to} className={`px-2 py-1.5 text-center font-medium rounded ${bg}`}>
                                {from === to ? '—' : val || ''}
                              </td>
                            )
                          })}
          ...
          <div className={`rounded-xl border overflow-hidden ${t.card} ${t.border}`}>
            <div className={`px-5 py-3 border-b ${t.border}`}>
              <h2 className={`text-sm font-semibold ${t.text}`}>🔄 재방문율</h2>
              <p className={`text-xs ${t.muted}`}>같은 지점 2회+ 방문 비율</p>
            </div>
            <div className="p-3 space-y-1.5">
              {revisitRates.slice(0, 12).map((row, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className={`text-sm font-medium w-28 truncate ${t.text}`}>{row.branch}</span>
                  ...
                  <span className={`text-xs font-bold w-10 text-right ${row.rate > 20 ? 'text-blue-400' : t.text}`}>{fmtPct(row.rate)}</span>
